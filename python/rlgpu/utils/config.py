...
# Note: This file includes code that was modified to support different robots
# and extend the CLI arguments for a wider hyper parameter search
...
def warn_task_name():
    raise Exception("Unrecognized task!\nTask should be one of: [Iiwa14, Franka, Jaco, UR5, DoosanH2017]")
...
def retrieve_cfg(args, use_rlg_config=False):
    if use_rlg_config:
        if args.task == "Iiwa14":
            return os.path.join(args.logdir, "iiwa14"), "cfg/train/rlg_iiwa14.yaml", "cfg/iiwa14.yaml"
        elif args.task == "DoosanH2017":
            return os.path.join(args.logdir, "doosan_h2017"), "cfg/train/rlg_doosan_h2017.yaml", "cfg/doosan_h2017.yaml"
        else:
            warn_task_name()
    else:
        if args.task == "Iiwa14":
            return os.path.join(args.logdir, "iiwa14"), "cfg/train/pytorch_ppo_iiwa14.yaml", "cfg/iiwa14.yaml"
        elif args.task == "Franka":
            return os.path.join(args.logdir, "franka"), "cfg/train/pytorch_ppo_franka.yaml", "cfg/franka.yaml"
        elif args.task == "Jaco":
            return os.path.join(args.logdir, "jaco"), "cfg/train/pytorch_ppo_jaco.yaml", "cfg/jaco.yaml"
        elif args.task == "UR5":
            return os.path.join(args.logdir, "ur5"), "cfg/train/pytorch_ppo_ur5.yaml", "cfg/ur5.yaml"
        elif args.task == "DoosanH2017":
            return os.path.join(args.logdir, "doosan_h2017"), "cfg/train/pytorch_ppo_doosan_h2017.yaml", "cfg/doosan_h2017.yaml"
        else:
            warn_task_name()
...
def load_cfg(args, use_rlg_config=False):
    ...
    if args.ent_coef != None:
        cfg_train["learn"]["ent_coef"] = args.ent_coef
    if args.gamma != None:
        cfg_train["learn"]["gamma"] = args.gamma
    if args.lam != None:
        cfg_train["learn"]["lam"] = args.lam

    if args.distance_reward_scale != None:
        cfg["env"]["distance_reward_scale"] = args.distance_reward_scale
    if args.obstacle_reward_scale != None:
        cfg["env"]["obstacle_reward_scale"] = args.obstacle_reward_scale
    if args.reach_goal_bonus != None:
        cfg["env"]["reach_goal_bonus"] = args.reach_goal_bonus

    if args.curr_modus >= 0:
        cfg["env"]["curr_modus"] = args.curr_modus
    if args.curr_level > 0:
        cfg["env"]["curr_level"] = args.curr_level
...
def get_args(benchmark=False, use_rlg_config=False):
    custom_parameters = [
        ...
        {"name": "--task", "type": str, "default": "Iiwa14",
            "help": "Can be Iiwa14, Franka, Jaco, UR5, DoosanH2017"},
        ...
        {"name": "--logdir", "type": str, "default": "logs/",
            "help": "Directoy to store log files"},
        ...
        {"name": "--cfg_train", "type": str, "default": "Base",
            "help": "Path to the cfg file if not using default"},
        ...
        {"name": "--curr_level", "type": int, "default": 0,
            "help": "The starting curriculum"},
        {"name": "--curr_modus", "type": int, "default": -1,
            "help": "The curriculum methodology to use"},

        {"name": "--ent_coef", "type": float, "default": None,
            "help": "The entropy coefficient"},
        {"name": "--gamma", "type": float, "default": None,
            "help": "Gammea - the discount factor"},
        {"name":"--lam", "type": float, "default": None,
            "help": "Trade-off of bias vs variance for Generalized Advantage Estimator"},

        {"name": "--distance_reward_scale", "type": float, "default": None,
            "help": "The value used for scaling the distance reward function"},
        {"name": "--obstacle_reward_scale", "type": float, "default": None,
            "help": "The value used for scaling the obstacle avoidance reward function"},
        {"name": "--reach_goal_bonus", "type": int, "default": None,
            "help": "The bonus reward given to the agent when it reaches the goal"},

        # arguments for curriculum_test.py
        {"name": "--interior", "action": "store_true",
            "help": "Visualizses points on the interior of the curriculum"},
        {"name": "--boundary", "action": "store_true",
            "help": "Visualizses points only on the boundary of the curriculum"},

        # argument for dist_rot_test.py
        {"name": "--random_goal", "type": int, "default": 1,
            "help": "Chose a random goal between 1-4"},

        # arguments for generate_poses.py
        {"name": "--record", "type": bool, "default": False,
            "help": "Arg for generate_poses.py to record samples for curriculum"},
        {"name": "--split", "type": bool, "default": False,
            "help": "Arg for generate_poses.py to split the recorded samples into curriculums"},
        {"name": "--num_samples", "type": int, "default": None,
            "help": "Arg for generate_poses.py the number of samples in millions to record"}
            ]
